// THIS IS A FORK FROM
// https://github.com/instea/react-native-popup-menu/blob/master/src/renderers/Popover.js
...
function getBottomPrice(hOptions, vOptions) {
  ...
  const sideOffset = Math.max(
    0,
    hOptions.tPos + hOptions.tDim + hOptions.oDim - hOptions.wDim,
  );
  return centerOffset + sideOffset;
}
...
function getRightPrice(hOptions, vOptions) {
  ...
  const sideOffset = Math.max(
    0,
    vOptions.tPos + vOptions.tDim + vOptions.oDim - vOptions.wDim,
  );
  return centerOffset + sideOffset;
}
